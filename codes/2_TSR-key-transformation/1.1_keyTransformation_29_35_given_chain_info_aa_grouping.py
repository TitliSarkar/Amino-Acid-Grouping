# -*- coding: utf-8 -*-
"""
Created on Tue Oct 25 10:40:07 2020

@author: titli
"""

## Code for generating TSR keys from pdb files (with amino acid grouping)
## Input : .pdb files
## Output: .triplets_aa_grp and .keys_aa_hrp files 
##			.keys files contains key - freqofthekey for each protein, generated with amino acid groupung
##			.triplets files contains details of each key and corresponding TSR for each protein, generated with amino acid grouping
...
parser.add_argument("path", help="enter path to the input file", type=str) # ./../Dataset/
parser.add_argument("start", help="enter starting of list to be processed", type=int) 
parser.add_argument("stop", help="enter ending of list to be processed", type=int) 
...
df = pd.read_csv('sample_details.csv',sep=',',header=0)
chain_dict = dict([(p,c) for p,c in zip(df.protein, df.chain)])

start = args.start
stop = args.stop
files = df['protein'].tolist()[start:stop]
...
aminoAcidCode=open("aminoAcidCode_grouping_type0.txt","r")
...
def parallelcode(fileName, chain):
    ...
    outFile2 = open(data_dir+subdir+fileName+".keys_29_35_aa_grp_0", "w")
    fileTriplets = open(data_dir+subdir+fileName+".triplets_29_35_aa_grp_0","w")
    ...
    print ("FILENAME=",fileName,'\t',"NUM OF AMINOACIDS=",protLen,'\t', "TIME TAKEN: ", round(time.time()-start_time), " sec.")
    outFile2.close()
    fileTriplets.close()
    ## end of parallelcode()
...
Parallel(n_jobs=num_cores, verbose=50)(delayed(parallelcode)(fileName,chain_dict[fileName].upper())for fileName in files)

print("Code end 29_35.")
