...
## Instruction: 
	# Before running this code, you need to create a folder named "Dataset_protname1chain_protname2chain_all" and place the two proteins in comparison in that folder

## Code for TSR key generation for two proteins
## Input : .pdb files
## Output: .keys_29_35_aa_grp_0 and .triplets_29_35_aa_grp_0 files with TSR keys and key details, .residues file for residue list
...
print("#of proteins = ", len(files), files)
...
#def parallelcode(fileName, chain):
    ...
    for i in inFile:
        ...
        if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" and i[21:22].strip()==chain:
            ...
            counter+=1
    res_list = [x.strip() for x in list(seq_number.values())]
    print(len(res_list))
    with open(data_dir+subdir+fileName+".residues", "wb") as fr:
        pickle.dump(res_list, fr)
    fr.close()
    protLen=len(yCord)
    initialLabel=[]
    sortedLabel=[]
